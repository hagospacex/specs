Pod::Spec.new do |s|
   ...
   s.version               = "0.0.13-dev"
   s.summary               = "学习中心静态库sdk，用于外部接入"
   s.homepage              = "http://www.hqwx.com"
   s.license               = "MIT"
   s.author                = {"litianqi" => "litianqi@hqwx.com"}
   
   s.source                = {:http => "http://yyci2020.oss-cn-shenzhen.aliyuncs.com/yyqapm/ios/HQLearnCenterExternal/0.0.13-dev/HQLearnCenterExternal.zip"}
   s.platform              = :ios, "9.0"
   s.requires_arc  = true
...
s.dependency 'HqAppArchitect', '~>0.1.13'
s.dependency 'HqEbookViewSDK', '~>3.0.2'
s.dependency 'HqMoviePlayerSDK', '~>1.2.23'
